...
// The multiple-webhook fan-out must still deliver to every instance URL.
func TestFansOutToMultipleInstanceWebhooks(t *testing.T) {
	var a, b int64
	...
	p := newTestProducer(t, "")
	_ = p.Produce("inst.message", []byte(`{}`), serverA.URL+","+serverB.URL, "i1")
