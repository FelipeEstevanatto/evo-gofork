...
//
// Answers the two questions an operator actually has about a proxy: does traffic
// get through, and which IP does it come out of (i.e. is it really hiding the
// server, or silently passing traffic through). It also checks whether WhatsApp
// itself is reachable through it, since a proxy can reach the open internet and
// still be blocked by WhatsApp.
//
...
// the status code, since a bare GET is expected to be refused.
